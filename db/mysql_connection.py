...
class MySQLConnection:
    ...
    def __init__(self, host: str, user: str, password: str, name: str, port: str) -> None:
        ...
        if MySQLConnection.__instance is None:
            MySQLConnection.__instance = mysql.connect(host=self.host, user=self.user, passwd=self.password, database=self.name, port=self.port, auth_plugin='mysql_native_password')
        else:
